...
def create_feedback(db, box_id, text):
    try:
        ...
        logger.warning("Feedback moderation failed for box_id=%s: %s", box_id, e)
        raise HTTPException(
    ...
    try:
        ...
        db.rollback()
        logger.critical("Database failure while creating feedback for box_id=%s", box_id, exc_info=exc)
        raise
