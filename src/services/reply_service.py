...
def create_reply(db, feedback_id, text):
    try:
        ...
        logger.warning("Reply moderation failed for feedback_id=%s: %s", feedback_id, e)
        raise HTTPException(
    ...
    try:
        ...
        db.rollback()
        logger.critical("Database failure while creating reply for feedback_id=%s", feedback_id, exc_info=exc)
        raise
